...
export async function getAnalyticsSummary() {
  const supabase = createSupabaseAdminClient();
  const empty = { totalVisitors: 0, visitorsToday: 0, onlineVisitors: 0, totalPageViews: 0 };
  ...
  const { data, error } = await supabase.from("visitor_stats").select("visitor_hash, viewed_at");
  if (error) throw error;
  const rows = data ?? [];
  return {
